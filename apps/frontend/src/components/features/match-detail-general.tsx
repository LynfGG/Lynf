...
/**
 * The General tab: every participant Riot reported, grouped by team, each with
 ...
 *
 * Reads exactly what the match history tranche already stored — `match.participants` —
 * so opening this tab never calls Riot.
 */
export default function MatchDetailGeneral({
    participants,
    viewedPuuid,
    version,
    catalogue,
    language,
}: Readonly<MatchDetailGeneralProps>) {
    const { t } = useTranslation('summoner');
